...
class XcoinKit : AbstractKit, IInstantTransactionDelegate, BitcoinCore.Listener {
    ...
    private val xcoinStorage: DashStorage
    private val instantSend: InstantSend
    private val xcoinTransactionInfoConverter: DashTransactionInfoConverter
    ...
    constructor(
        ...
        val xcoinDatabase = DashKitDatabase.getInstance(context, getDatabaseName(networkType, walletId, syncMode))
        val initialSyncUrl: String

        val coreStorage = Storage(coreDatabase)
        xcoinStorage = DashStorage(xcoinDatabase, coreStorage)
        ...
        val paymentAddressParser = PaymentAddressParser("xcoin", removeScheme = true)
        val instantTransactionManager = InstantTransactionManager(xcoinStorage, InstantSendFactory(), InstantTransactionState())
        val initialSyncApi = InsightApi(initialSyncUrl)

        xcoinTransactionInfoConverter = DashTransactionInfoConverter(instantTransactionManager)
        ...
        bitcoinCore = BitcoinCoreBuilder()
                ...
                .setTransactionInfoConverter(xcoinTransactionInfoConverter)
        ...
        val quorumListManager = QuorumListManager(xcoinStorage, QuorumListMerkleRootCalculator(merkleRootCreator), QuorumSortedList())
        val masternodeListManager = MasternodeListManager(xcoinStorage, masternodeListMerkleRootCalculator, masternodeCbTxHasher, MerkleBranch(), MasternodeSortedList(), quorumListManager)
        ...
        val bls = BLS()
        val transactionLockVoteValidator = TransactionLockVoteValidator(xcoinStorage, singleHasher, bls)
    ...
    fun xcoinTransactions(fromUid: String? = null, limit: Int? = null): Single<List<DashTransactionInfo>> {
        return transactions(fromUid, limit).map {
            it.mapNotNull { it as? DashTransactionInfo }
        }
    }

    fun getXcoinTransaction(hash: String): DashTransactionInfo? {
    ...
    override fun onUpdateInstant(transactionHash: ByteArray) {
        val transaction = xcoinStorage.getFullTransactionInfo(transactionHash) ?: return
        val transactionInfo = xcoinTransactionInfoConverter.transactionInfo(transaction)
    ...
    companion object {
        ...
        private fun getDatabaseName(networkType: NetworkType, walletId: String, syncMode: SyncMode) =
                "Xcoin-${networkType.name}-$walletId-${syncMode.javaClass.simpleName}"
